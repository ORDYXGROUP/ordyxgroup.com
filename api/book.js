...
//   RESEND_API_KEY, BOOKING_SECRET, SITE_URL
//   STEFAN_EMAIL, FROM_EMAIL (e.g. sessions@ordyxgroup.com)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const stefanEmail = process.env.STEFAN_EMAIL || 'stefan@ordyxgroup.com';
    ...
    if (supabaseUrl && supabaseKey) {
      const leadRes = await fetch(`${supabaseUrl}/rest/v1/leads`, {
        ...
        headers: {
          ...
          'Content-Type':  'application/json',
          'Prefer':        'return=representation',   // ← returns the new row with id
        },
        body: JSON.stringify({
          name,
      ...
      console.log('[book] Supabase not configured:', { name, company, email, date, time });
    }

    // ── 2. Build magic links ─────────────────────────────────────────────────
    const tokenId = leadId || email;           // use lead UUID if available, else email
...
<div class="w">
  ...
  <div class="foot">
    ORDYX GROUP · Internal Notification<br>
    Clicking Confirm will send the client a calendar invite automatically.
  </div>
</div>
</body></html>`,
        });
      } catch (e) {
        console.error('[book] Stefan approval email error:', e.message);
      }
...
<div class="w">
  ...
  <div class="foot">Questions? Reply directly to this email.<br>ORDYX GROUP · Frankfurt</div>
        ...
        console.error('[book] Client holding email error:', e.message);
      }
    }
